...
class TestExceptions:
    ...
    def test_invalid_element_state_exception(self, driver):
        # Test for invalid element state exception
        exceptions_page = ExceptionsPage(driver)
        exceptions_page.open()
    ...
    def test_stale_element_reference_exception(self, driver):
        # Test for stale element reference exception
        exceptions_page = ExceptionsPage(driver)
        exceptions_page.open()
        exceptions_page.add_second_row()
        # Verify that the instruction element is no longer displayed
        assert not exceptions_page.are_instructions_displayed(), "Instruction text element should not be displayed"

    @pytest.mark.exceptions
    def test_timeout_exception(self, driver):
        # Test for timeout exception by waiting for element visibility
        exceptions_page = ExceptionsPage(driver)
        exceptions_page.open()
        exceptions_page.add_second_row()
        # Verify that the second row input is displayed
